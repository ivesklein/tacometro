...
//recommended gear blinking status
bool gl = false;
//acc blinking status
bool al = false;
...
void setup() {
  ...
  TM1.begin(17, 18, 4);       //  clockpin(A2), datapin(A3), #digits
  TM1.displayClear();
...
void updateVel(){
  floatToChar(speed, numStr);
  TM1.displayPChar(numStr);
}

void floatToChar(float input, char numStra[5]) {
    int floored = (int)input; // Floor the input float
    int digits = 0;
    ...
    // if is on and not moving show 0
    if(rpm>0){
      if(digits == 0){
        digits = 1;
      }
    ...
    // Fill in spaces at the beginning of the array
    for (int i = 0; i < spaces; i++) {
        numStra[i] = ' '; // Insert spaces
    }

    // Convert integer to string without sprintf
    itoa(floored, &numStra[spaces], 10); // Convert integer to string starting from index 'spaces'
    numStra[4] = '\0'; // Null-terminate the string
}
...
void turnOnLights(){
  ...
  digitalWrite(lights2, true);
}

void turnOffLights(){
  digitalWrite(lights, false);
  digitalWrite(lights2, false);
}
